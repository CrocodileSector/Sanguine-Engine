...
struct ClientDescription
{
	...
	ClientDescription()
		: PeerID(0), AvatarID(0), Position(1.0f), Direction(1.0f)
	{

	}

	ClientDescription(uint32_t peerID, uint32_t avatarID, glm::vec3 pos, glm::vec3 dir)
		: PeerID(peerID), AvatarID(avatarID), Position(pos), Direction(dir)
	{
	...
	namespace net
	{
		...
		struct message
		{
			...
			friend message<T>& operator << (message<T>& msg, const DataType& data)
			{
				...
				if constexpr (std::is_same_v<DataType, ChatMessage>)
				{
			...
			friend message<T>& operator << (message<T>& msg, const ChatMessage& data)
			{
				uint32_t offset = 0;

				msg.body.resize(2 * sizeof(uint32_t) + data.MessageLen);

				std::memcpy(msg.body.data(), &data.PeerID, sizeof(uint32_t));
				offset += sizeof(uint32_t);

				std::memcpy(msg.body.data() + offset, &data.MessageLen, sizeof(uint32_t));
				offset += sizeof(uint32_t);

				for (uint32_t i = 0; i < data.MessageLen; ++i)
				{
					msg.body[i + offset] = data.Message[i];
				}
			...
			friend message<T>& operator >> (message<T>& msg, ChatMessage& data)
			{
				uint32_t offset = 0;

				uint32_t peerID = 0;
				std::memcpy(&peerID, msg.body.data(), sizeof(uint32_t));
				offset += sizeof(uint32_t);

				uint32_t messageLen = 0;
				std::memcpy(&messageLen, msg.body.data() + offset, sizeof(uint32_t));
				offset += sizeof(uint32_t);

				char* message = new char[messageLen];
				for (uint32_t i = 0; i < messageLen; ++i)
				{
					message[i] = msg.body[i + offset];
				}

				data.PeerID = peerID;
				data.MessageLen = messageLen;
		...
		public:
			...
			bool Connect(const std::string& host, const uint16_t port)
			{
				...
				catch (std::exception& e)
				{
					...
					SG_CORE_ERROR(std::string(std::string("Detected connection error: ") + e.what()).c_str());
					return false;
				}
